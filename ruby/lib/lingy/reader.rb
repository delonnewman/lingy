...
module Lingy
  class Reader
    ...
    START_SYMBOL_PAT = %r{[A-Za-z_\-*/+=?\^<>%$\#@!.]}
    SYMBOL_PAT       = %r{[A-Za-z0-9_\-*/+=?^<>%$#@!.:\[\]]}
    DIGIT_PAT        = /\d/
    DOUBLE_QUOTE     = '"'
    OPEN_PAREN       = '('
    CLOSE_PAREN      = ')'
    ...
    def next!
      ...
      if current_token == ';'
        next_token! until current_token == NEWLINE || current_token == RETURN
        next_token!
        return self
      end
    ...
    def read_array!
      ...
      first_line = line
      until current_token == CLOSE_BRACKET || eof?
        array << next!
        next_token! while whitespace?(current_token)
    ...
    def read_hash!
      ...
      array = []
      first_line = line
      until current_token == CLOSE_BRACE || eof?
        array << next!
        next_token! while whitespace?(current_token)
    ...
    def read_keyword!
      ...
      until !symbol_token?(current_token) || eof?
        buffer << current_token
        next_token!
      end
